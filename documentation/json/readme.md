...
The input can contain competition structures, including descriptors - which describe a structure to build (e.g. "round-robin" which generates an all-play-all round-robin structure) as well as events (e.g. setting the result of a particular phase or match). This, trivially, allows you to turn a 20 long entry list (for example) into a fully realised 380 match league where everyone plays home and away.
